...
ensure_api_env_defaults() {
  ...
  local voice_agent_domain="${VOICE_AGENT_DOMAIN:-agent.coziyoo.com}"
  local cors_default="${API_CORS_ALLOWED_ORIGINS:-https://${admin_domain},http://${admin_domain},https://${voice_agent_domain},http://localhost:8081,http://localhost:5173,http://localhost:19006,http://localhost:3001}"
  local defaults=(
    "PGHOST=127.0.0.1"
    "PGPORT=5432"
